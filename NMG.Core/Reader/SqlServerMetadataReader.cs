...
namespace NMG.Core.Reader
{
    ...
    public class SqlServerMetadataReader : IMetadataReader
    {
        ...
        public Column ColumnNormalizer(Column column)
        {
            if (column.Name.ToLower().Contains("email"))
            {
                column.RegX = "^[a-z0-9_\\+-]+(\\.[a-z0-9_\\+-]+)*@[a-z0-9-]+(\\.[a-z0-9-]+)*\\.([a-z]{2,4})$";
            }
            if (column.Name.ToLower().Contains("percent"))
            {
                column.BeginOfRange = 0;
                column.EndOfRange = 100;
            }
            ...
            if (column.DataType.ToLower().StartsWith("nvarchar")
                || column.DataType.ToLower().StartsWith("nchar")
                || column.DataType.ToLower().StartsWith("char")
                || column.DataType.ToLower().StartsWith("varchar"))
            {
                column.InLookUpLabel = true;
            }
            return column;
        }
                ...
                using (conn)
                {
                    using (var tableDetailsCommand = conn.CreateCommand())
                    {
                        tableDetailsCommand.CommandText = string.Format(
                            @"
						SELECT distinct c.column_name, c .data_type, c.is_nullable, tc.constraint_type, c.numeric_precision, c.numeric_scale, c.character_maximum_length, c.table_name, c.ordinal_position, tc.constraint_name,  COLUMNPROPERTY(object_id('{0}'), c.column_name, 'IsIdentity') as IsIdentity 
                        ...
                        using (var sqlDataReader = tableDetailsCommand.ExecuteReader(CommandBehavior.Default))
                        {
                            while (sqlDataReader.Read())
                            {
                                ...
                                if (isPrimaryKey)
                                    isIdentity = (sqlDataReader["IsIdentity"] as int?) == 1 ? true : false;
                                ...
                                var newColumn = new Column
                                                {
                                                    ...
                                                    IdentitySpecification = isIdentity,
                                ...
                                try
                                {
                                    var cn = ColumnNormalizer(newColumn);
                                    var c = MetaDataGenerator.UpdateColumnMetaData(table, cn);
                                    columns.Add(c);
                                }
                                catch (Exception ex)
                                {
            ...
            try
            {
                using (conn)
                {
                    ...
                    while (sqlDataReader.Read())
                    {
                        ...
                        tables.Add(new Table { Name = tableName, DatabaseName = builder.InitialCatalog });
                    }
                }
        ...
        private List<ForeignKey> DetermineForeignKeyReferences(Table table)
        {
