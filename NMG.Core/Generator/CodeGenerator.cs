    ...
    public class CodeGenerator : AbstractGenerator
    {
        ...
        public CodeCompileUnit GetCompileUnit(string className)
        {
            ...
            foreach (var pk in Table.PrimaryKey.Columns)
            {
                ...
                var prop = codeGenerationHelper.CreateAutoProperty(mapFromDbType.ToString(), Formatter.FormatText(pk.Name), Table.Name, this.appPrefs, appPrefs.UseLazy);
            ...
            foreach (var column in Table.Columns.Where(x => x.IsPrimaryKey != true))
            {
                ...
                var prop = codeGenerationHelper.CreateAutoProperty(mapFromDbType, Formatter.FormatText(column.Name), column.IsNullable, column.DataLength, Table.Name, column, this.appPrefs, appPrefs.UseLazy);
                newType.Members.Add(prop);
            }
        ...
        {
            if (!Directory.Exists(string.Format(@"{0}\Mappings", filePath)))
            {
                Directory.CreateDirectory(string.Format(@"{0}\Mappings", filePath));
            }
            string fileName = string.Format(@"{0}\Mappings\", filePath) + className;
