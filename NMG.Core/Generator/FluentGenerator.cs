...
namespace NMG.Core.Generator
{
    public class FluentGenerator : AbstractCodeGenerator
    {
        ...
        public FluentGenerator(ApplicationPreferences appPrefs, Table table)
            : base(appPrefs.FolderPath, "Mapping", appPrefs.TableName, appPrefs.NameSpace, appPrefs.AssemblyName, appPrefs.Sequence, table, appPrefs)
        {
        ...
        public CodeCompileUnit GetCompleteCompileUnit(string className)
        {
            ...
            {
                if (fk.References + "Map" != className)
                {
                ...
                {
                    constructor.Statements.Add(new CodeSnippetStatement(string.Format(TABS + "References(x => x.{0}).Column(\"{1}\");", Formatter.FormatSingular("Parent" + fk.UniquePropertyName), fk.Name)));
                }
            }
    ...
    public class OneToMany
    {
        ...
        {
            var hasManySnippet = string.Format("HasMany(x => x.{0})", Formatter.FormatPlural(hasMany.Reference + hasMany.ReferenceColumn));
