...
namespace NHibernateMappingGenerator
{
    public partial class App : Form
    {
        ...
        public App()
        {
            ...
            var db = new NMG.Core.DataManager.DataContext.NMGTableMetaDataDataContext();
        ...
        private void PopulateTablesAndSequences()
        {
            ...
            try
            {
                ...
                var tables = metadataReader.GetTables(ownersComboBox.SelectedItem.ToString()).Where(u => !u.Name.ToLower().Equals("sysdiagrams"));
        ...
        private void GenerateClicked(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (ResourceGenerationCheckBox.Checked)
                {
                    if (!Directory.Exists(string.Format(@"{0}\Resource", folderTextBox.Text)))
                    {
                        Directory.CreateDirectory(string.Format(@"{0}\Resource", folderTextBox.Text));
                    }
                    ResourceGenerator.GenerateMessageResources(folderTextBox.Text, CommonResourseTextBox.Text, errorResourceTextBox.Text, "App_Headers", resouceReferenceTextBox.Text, dbName, tables);
                }
        ...
        private void SaveMetadatButton_Click(object sender, EventArgs e)
        {
            errorLabel.Text = string.Empty;
            var selectedItems = tablesListBox.SelectedItems;
            if (selectedItems.Count == 0)
            {
                errorLabel.Text = @"Please select table above to save the metadatas.";
                return;
            }
            try
            {
                foreach (var selectedItem in selectedItems)
                {
                    errorLabel.Text = string.Format("Saving {0} Metadatas ...", selectedItem);
                    var table = (Table)selectedItem;
                    MetaDataGenerator.UpdateAllDatabaseMetaData(table);
                }
                errorLabel.Text = @"Saved all metadatas successfully.";
            }
            catch (Exception ex)
            {
                errorLabel.Text = ex.Message;
        ...
        private void GenerateAllClicked(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                finally
                {
                    if (ResourceGenerationCheckBox.Checked)
                    {
                        var table = (Table)tablesListBox.Items[0];
                        metadataReader.GetTableDetails(table, ownersComboBox.SelectedItem.ToString());
                        dbName = table.DatabaseName;
                        if (!Directory.Exists(string.Format(@"{0}\Resource", folderTextBox.Text)))
                        {
                        ...
                        ResourceGenerator.GenerateMessageResources(folderTextBox.Text, CommonResourseTextBox.Text, errorResourceTextBox.Text, "App_Headers", resouceReferenceTextBox.Text, table.DatabaseName, Tables);
                    }
                    if (GenerateViewCheckBox.Checked)
                    {
                        ViewGenerationHelper.GnerateMenuView(folderTextBox.Text, dbName);
                    }
                    Cursor.Current = Cursors.Default;
                }
            }
        ...
        {
            if (!table.Name.ToLower().Equals("sysdiagram"))
            {
            ...
            var applicationPreferences = new ApplicationPreferences
                                             {
                                                 ...
                                                 UseLazy = useLazyLoadingCheckBox.Checked,
                                                 IncludeForeignKeys = includeForeignKeysCheckBox.Checked,
                                                 AttributeGeneration = aatributeGenerationCheckBox.Checked,
                                                 ResourceReference = resouceReferenceTextBox.Text,
                                                 DisplayNameForAttribute = displayNameAttributeTextBox.Text,
                                                 TablePrefix = tablePreTextBox.Text,
                                                 TablePost = tablePostTextBox.Text,
                                                 MaxLengthName = maxLengthTextBox.Text,
                                                 Required = requieredTextBox.Text,
                                                 ErrorResourse = errorResourceTextBox.Text,
                                                 ResourceGeneration = ResourceGenerationCheckBox.Checked,
                                                 RegxError = RegxErrorTextBox.Text,
                                                 RangeError = RangeErrorTextBox.Text,
                                                 ControllerGeneration = GenearteControllerCheckBox.Checked,
                                                 InheritBaseController = InheritBaseControllerCheckBox.Checked,
                                                 AuthorizeEnable = AuthorizeCheckBox.Checked,
                                                 GenerateCRUD = CreateCrudCheckBox.Checked,
                                                 ControllerNamespace = ControllerNamespaceTextBox.Text,
                                                 ControllerReferences = RefrenceRichTextBox.Text,
                                                 GenerateLookUps = GenerateLookupCheckBox.Checked,
                                                 GenerateSearchModels = SearchModelGenerationCheckBox.Checked,
                                                 IncludePaging = IncludePagingCheckBox.Checked,
                                                 GenerateViews = GenerateViewCheckBox.Checked,
                                                 UseAjax = UseAjaxCheckBox.Checked,
                                                 CommonResourseName = CommonResourseTextBox.Text,
                                                 WithKendo = KendoCheckBox.Checked,
                                                 IncludeVirtual = VirtualcheckBox.Checked,
                                                 AnnotationFile = AnnotationCheckBox.Checked,
                                                 DALFiles = DALFiles.Checked,
                                                 DALNamespace = DALNameSpace.Text,
                                                 DALReferences = DALReferencesTextBox.Text,
                                                 DBUtilityName = DBUtilitytextBox.Text,
                                                 DataContextName = DataContextTextBox.Text,
                                                 WithSortExpression = SortExpressionCheckBox.Checked,
                                                 SystemDefaults = DfaultCheckBox.Checked,
                                                 ORMView = dbmlRadioButton.Checked ? 1 : 2,
                                                 Bootstrap = bootstrapCheckBox.Checked
                                             };
            //applicationPreferences.DALReferences += string.Format("using {0};using {0}.Data;using APS.SIS.Model;", nameSpaceTextBox.Text);
